...
async def connect(sid: str, environ: Dict[str, Any]) -> None:
    app_logger.info(f"[connect] {sid} подключился")
...
async def disconnect(sid: str) -> None:
    ...
    if user_id:
        ...
        app_logger.info(f"[disconnect] {user_id} вышел из комнаты {room}")
    else:
        app_logger.info(f"[disconnect] {sid} отключился без user_id")
...
async def join(sid: str, data: Dict[str, Any]) -> None:
    ...
    if not token:
        app_logger.warning(f"[join] Нет токена от {sid}")
        return
    ...
    try:
        ...
        app_logger.warning(f"[join] Невалидный токен от {sid}: {e}")
        return

    connected_users[sid] = user_id
    room = f"user_{user_id}"
    await sio.enter_room(sid, room)
    app_logger.info(f"[join] Пользователь {user_id} вошёл в комнату {room}")
...
async def message(sid: str, data: Dict[str, Any]) -> None:
    ...
    if not sender_id:
        app_logger.warning(f"[message] {sid} не аутентифицирован")
        return

    receiver_id = data.get("receiver_id")
    text = data.get("text", "").strip()
    if not text or not receiver_id:
        app_logger.warning(f"[message] Неверные данные от {sender_id}")
        return
    ...
    app_logger.info(f"[message] {sender_id} -> {receiver_id}")
